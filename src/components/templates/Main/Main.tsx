...
export const Main = () => {
  const [token, setToken] = React.useState<LoginForm>({login:false, userType:''});
    ...
    <ProviderContext>
      ...
      <AdminMenu />
        ...
        token.login ? 
          <>
          {
          token.userType === 'admin1' ? <><AdminMenu /><Redirect to="/admin_menu" path="/admin_menu" /></>
          : 
          <><UserMenu /><Redirect to="/user_menu" path="/user_menu" /></>
          }
          </>
          :
          <>
